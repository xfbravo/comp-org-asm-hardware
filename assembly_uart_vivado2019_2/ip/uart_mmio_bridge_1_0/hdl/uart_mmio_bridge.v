...
module uart_mmio_bridge #(
    ...
    output wire        uart_rx_overrun,
    output wire [3:0]  uart_rx_level,
    ...
    wire tx_room = !(uart_tx_busy || uart_tx_start);
    assign uart_rx_ack = rst_n && mmio_read && mmio_addr == 32'h4000_0004;
    uart_controller #(.CLOCK_HZ(CLOCK_HZ), .BAUD_HZ(BAUD_HZ)) u_uart (
        ...
        .frame_error(uart_frame_error), .rx_overrun(uart_rx_overrun), .rx_level(uart_rx_level)
    );
    always @(*) begin
        case (mmio_addr)
            32'h4000_0004: mmio_rdata = {24'd0, uart_rx_data};
            32'h4000_0008: mmio_rdata = {23'd0, uart_rx_level, (uart_rx_level >= 4'd7), uart_rx_overrun, uart_frame_error, uart_rx_ready, tx_room};
            default:      mmio_rdata = 32'd0;
        endcase
    end
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            uart_tx_start <= 1'b0; uart_tx_data <= 8'd0;
            seg7_we <= 1'b0; seg7_value <= 32'd0;
        end else begin
            uart_tx_start <= 1'b0; seg7_we <= 1'b0;
            if (mmio_write && mmio_addr == 32'h4000_0000 && tx_room) begin
