...
module asm_cpu_core #(
    ...
    parameter UART_CLOCK_HZ = 50_000_000,
    parameter UART_BAUD_HZ = 115200
)(
    input  wire        clk,
    input  wire        rst_n,
    input  wire        uart_rxd,
    ...
    output wire [31:0] debug_uart_status
);
    reg [31:0] pc;
    wire [31:0] pc_next;
    wire [31:0] if_instr;
    ...
    wire [31:0] mem_read_data_core;
    wire [31:0] mem_read_data;
    wire [31:0] mem_debug_word0;

    wire mmio_sel = (ex_mem_alu_result[31:28] == 4'h4);
    wire mmio_rd = ex_mem_valid && ex_mem_mem_read && mmio_sel;
    wire mmio_wr = ex_mem_valid && ex_mem_mem_write && mmio_sel;
    wire [31:0] mmio_rdata;
    wire [31:0] dmem_addr = ex_mem_alu_result;
    wire [31:0] dmem_wdata = ex_mem_store_data;
    wire [1:0]  dmem_size = ex_mem_mem_size;
    ...
    uart_mmio_bridge #(
        ...
        .clk(clk), .rst_n(rst_n),
        .mmio_read(mmio_rd), .mmio_write(mmio_wr), .mmio_addr(dmem_addr),
        .mmio_wdata(dmem_wdata), .mmio_rdata(mmio_rdata),
    ...
    data_mem #(
        ...
        .clk(clk),
        .rst_n(rst_n),
        .mem_read(ex_mem_mem_read && !mmio_sel),
        .mem_write(ex_mem_mem_write && !mmio_sel),
        .mem_size(ex_mem_mem_size),
        .load_unsigned(ex_mem_load_unsigned),
        .addr(ex_mem_alu_result),
        .write_data(ex_mem_store_data),
        .read_data(mem_read_data_core),
        .debug_word0(mem_debug_word0)
    );

    assign mem_read_data = mmio_sel ? mmio_rdata : mem_read_data_core;
